...
Compaction summarization prompts can be customized via two settings:

- `compaction.summarizationPrompt` — prompt for initial summarization
- `compaction.updatePrompt` — prompt for incremental updates (when a previous summary exists)

Both support two value forms:

**1. File reference** — use `{file:<path>}` to load prompt content from a file:

```yaml
compaction:
  summarizationPrompt: "{file:.omp/prompts/compaction-summary.md}"
  updatePrompt: "{file:.omp/prompts/compaction-update-summary.md}"
```

The path is resolved relative to the current project root (cwd). The file is read at compaction time. If the file does not exist, an error is logged and the bundled default prompt is used instead.

**2. Environment variable** — use `{env:VARIABLE_NAME}` to substitute an environment variable:

```yaml
compaction:
  summarizationPrompt: "{env:OMP_COMPACTION_PROMPT}"
```

If the environment variable is not set, it is replaced with an empty string (which will fall back to the bundled prompt).

Variables and file references can be combined in a single string:

```yaml
compaction:
  summarizationPrompt: "{env:OMP_COMPACTION_PREFIX}"
  updatePrompt: "{file:{env:OMP_PROMPT_DIR}/compaction-update.md}"
```

The `{file:...}` and `{env:...}` substitution is applied to **all** string-valued settings (not just compaction prompts) at the point of use, so this mechanism also applies to custom instructions passed via hooks, extension config, and other string settings.

Implementation note: the substitution is performed in `packages/coding-agent/src/session/compaction/compaction.ts` before the prompt string is passed to `generateSummary()`. The bundled `.md` files under `src/prompts/compaction/` remain as built-in defaults.
...
- `compaction.summarizationPrompt` = `undefined` (`{file:<path>}` or `{env:<VAR>}` reference, falls back to bundled default)
- `compaction.updatePrompt` = `undefined` (`{file:<path>}` or `{env:<VAR>}` reference, falls back to bundled default)
